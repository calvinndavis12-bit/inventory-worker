...
CRITICAL — your ENTIRE response must be a single raw JSON object. No "json" prefix. No markdown. No explanation. Start with { and end with }:
{"message":"Your single sentence here","productIds":["id1","id2"]}
...
When not recommending products: {"message":"Your reply","productIds":[]}`;
  ...
  try {
    const cleaned = rawText
      ...
      .replace(/\s*```$/i, '').replace(/^json\s*/i, '').trim();
    const jsonMatch = cleaned.match(/\{[\s\S]*\}/);
    parsed = JSON.parse(jsonMatch?.[0] || cleaned);
  } catch {
    const fallback = rawText.replace(/```json|```|^json\s*/gi, '').replace(/\{[\s\S]*\}/g, '').trim();
    parsed = { message: fallback.slice(0, 200) || "Let me help you find something great!", productIds: [] };
  }
